...
def save_text_results(text, text_dir):
    file_name = '%s/real_sample.txt' % text_dir
    f = open(file_name, "w+")
    for sent in text:
        f.write(sent + "\n")
    f.close()

def save_img_results(imgs_tcpu, imgs_collection, prefix1, prefix2, image_dir, nrow=8):
