...
class MeasureFID:
    ...
    def __init__(self, args):
        ...
        self.repeat_times = 5
        self.select_epoch = [epoch for epoch in range()]
    ...
    def prepare_folders(self):
        ...
        for epoch in select_epoch:
            for re_ix in range(self.repeat_times):
                folder = os.path.join(args.eval_image_folder, "Epoch_%d_%d" % (epoch, re_ix))
                img_folders.append(folder)
        return img_folders

    def calculate_fid_given_paths(self, model_path, compared_path, select_image_paths, batch_size, cuda, dims):
        ...
        if not os.path.join(compared_path):
            raise RuntimeError('Invalid path: %s' % compared_path)

        results = []
        m1, s1 = _compute_statistics_of_path(compared_path, model, batch_size, dims, cuda)
        all_ts = 0
    ...
    def get_activations(self, images, model, batch_size=64, dims=2048, cuda=False, verbose=True):
...
def _compute_statistics_of_path(path, model, batch_size, dims, cuda):
    if path.endswith('.npz'):
        ...
        dataset = img_data.Dataset(path, None, transform)
...
def gen_dataset_imgs(dir_path, pickle_path, saving_dir, sampling_nums=30000, batch_size=48):
    # how to design the numpy
